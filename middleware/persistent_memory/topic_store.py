...
class TopicStore(ABC):
    ...
    def __init__(self, root_path: str) -> None:
        ...
        self.logger.info("%s initialised. Root path: %s", self.__class__.__name__, self._root)
    ...
    async def search_topics(
        ...
        :return: List of ``{"topic", "content", "score"}`` dicts, best first.
        """
        async with await self._lock_for(self._list_lock_key(namespace)):
            bucket: dict[str, str] = await self._read_bucket(namespace)
        results: list[dict[str, object]] = self._keyword_rank(bucket, query, limit)
    ...
    async def _run_post_access(
        ...
        # Callback is best-effort — any error (network, summariser SDK, etc.)
        # must not lose the content that is already on disk.
        except Exception as error:  # pylint: disable=broad-except
            self.logger.warning(
                "%s: post-access callback failed for topic '%s': %s. Keeping original content.",
