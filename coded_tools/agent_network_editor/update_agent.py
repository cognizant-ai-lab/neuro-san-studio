...
class UpdateAgent(CodedTool):
    ...
    async def async_invoke(self, args: Dict[str, Any], sly_data: Dict[str, Any]) -> Union[Dict[str, Any], str]:
        ...
        if not self.agents.get(the_agent_name):
            return "Error: agent_name not in the agent network"
        the_instructions: str = args.get("new_instructions")
        the_down_chains: list[str] = args.get("new_down_chains")
        the_top_agent: bool = args.get("top_agent")
        ...
        the_agent_network_str = self.update_agent(the_agent_name, the_instructions, the_down_chains, the_top_agent)
    ...
    def update_agent(
            ...
            new_down_chains: list[str] = None,
            top_agent: bool = None
    ) -> str:
        """
        Updates an agent to the hierarchy.
        """
        if new_instructions:
            self.agents[agent_name]["instructions"] = new_instructions
        if new_down_chains:
            self.agents[agent_name]["down_chains"] = new_down_chains
        if top_agent:
            self.agents[agent_name]["top_agent"] = top_agent
