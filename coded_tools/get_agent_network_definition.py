    ...
    def invoke(self, args: dict[str, Any], sly_data: dict[str, Any]) -> dict[str, Any] | str:
        ...
        :return:
            ...
            otherwise:
                ...
                "Error: <error message>"
        """
        # Try to get the definition from args first, and fall back to sly data in none is provided.
        network_def = args.get("agent_network_definition") or sly_data.get(AGENT_NETWORK_DEFINITION)
        if not network_def:
            return "Error: No agent network definition in sly data!"
        logger = logging.getLogger(self.__class__.__name__)
        logger.info(">>>>>>>>>>>>>>>>>>>Get Agent Network Definition>>>>>>>>>>>>>>>>>>")
