...
def strip_inlined_refs(node: Any, ref_to_frontman: dict[str, str]) -> Any:
    """Return a deep copy of `node` with each `/<ref>` string replaced by the front_man name of
    the external network it points to. Critically, the file STEM (used in the reference) can
    differ from the front_man name (e.g., `/agent_network_editor` -> `network_editor`).
    """
    if isinstance(node, str):
    ...
    if isinstance(node, dict):
        return {key: strip_inlined_refs(value, ref_to_frontman) for key, value in node.items()}
    if isinstance(node, list):
        return [strip_inlined_refs(item, ref_to_frontman) for item in node]
    return node
...
def collect_external_refs_in_agents(agents: list[dict[str, Any]]) -> set[str]:
    """Collect every `/<name>` external ref appearing in any agent's `tools`, `messages`, or
    `allow` field. Other fields are ignored because external-agent references don't legitimately
    appear elsewhere.
    """
    refs: set[str] = set()
    for agent in agents:
        if isinstance(agent, dict):
            collect_external_refs(agent.get("tools"), refs)
            collect_external_refs(agent.get("messages"), refs)
            collect_external_refs(agent.get("allow"), refs)
    return refs
...
def internalize_externals(
    ...
    try:
        ...
        external_refs: set[str] = collect_external_refs_in_agents(agents) - own_names
        ...
        for ext_name in sorted(external_refs):
            if ext_name in ref_to_frontman:
                ...
                continue
            ext_path = find_external_hocon(ext_name, search_paths)
            ext_agents, ext_front_man = internalize_externals(ext_path, search_paths, visited, ref_to_frontman)
            ...
            merge_external_agents(agents, ext_agents, own_names)
...
def build_combined_config(input_path: Path, search_paths: list[Path]) -> dict[str, Any]:
    ...
    agents = [strip_inlined_refs(agent, ref_to_frontman) for agent in agents]
    clean_inlined_agents(agents)
...
def _format_value(value: Any, current_indent: int) -> str:
    ...
    if isinstance(value, dict):
        return _format_dict(value, current_indent)
    if isinstance(value, list):
        return _format_list(value, current_indent)
    return _format_scalar(value)


def _format_dict(node: dict[str, Any], current_indent: int) -> str:
    ...
    for i, (key, value) in enumerate(items):
        rendered = _format_value(value, inner_indent)
...
def _format_list(node: list[Any], current_indent: int) -> str:
    ...
    if all(not isinstance(item, (dict, list)) for item in node):
        inline = "[" + ", ".join(_format_scalar(item) for item in node) + "]"
    ...
    for i, item in enumerate(node):
        rendered = _format_value(item, inner_indent)
...
def render_hocon(combined: dict[str, Any]) -> str:
    ...
    return _format_dict(combined, current_indent=0) + "\n"


class InternalizeAgentsCommand:  # pylint: disable=too-few-public-methods
    ...
    def run(self) -> int:
        ...
        if not self.input_path.is_file():
            ...
            return EXIT_ERROR
        try:
            combined = build_combined_config(self.input_path, self.search_paths)
            output_text: str = render_hocon(combined)
