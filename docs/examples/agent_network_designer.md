...
`agent_network_metadata` at all, or sends it as null (nsflow's manual save sends nothing as of nsflow 0.7.1), gets the
existing file's block kept in file mode; sending the key, even empty, makes the block client-owned and
