...
The `neuro-san-studio` console script bundles a handful of subcommands for running the server and
managing a Neuro SAN Studio project. The default subcommand is `run`, so passing only `run`-style
flags is equivalent to invoking `neuro-san-studio run`.

## Subcommands

| Subcommand | Purpose |
|---|---|
| `run` | Start the Neuro SAN server and client (default). |
| `init` | Scaffold a starter project in the current directory. |
| `check-llm-keys` | Validate LLM API keys and other critical environment variables. |
